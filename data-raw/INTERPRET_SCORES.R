interpret_scores <- dimension_scores |>
  dplyr::filter(
    survey %in% c("GD", "GAD7", "BRS", "RDA", "MOCA")
  ) |>
  dplyr::filter(
    dimension %in% c("Total Score RDA", "Total Score BRS",
                     "Total Score GD", "Total Score GAD7",
                     "Total Score MOCA")
  ) |>
  dplyr::select(!dimension)|>
  dplyr::mutate(
    Interpretation = dplyr::case_when(
      survey == "GD" ~ dplyr::case_when(
        score <= 4 ~ "Normal",
        score < 9 ~ "Mild Depression",
        score < 12 ~ "Moderate Depression",
        score >= 12 ~ "Severe Depression"
      ),
      survey == "GAD7" ~ dplyr::case_when(
        score < 5 ~ "Minimal Anxiety",
        score <= 9 ~ "Mild Anxiety",
        score <= 14 ~ "Moderate Anxiety",
        score > 14 ~ "Severe Anxiety"
      ),
      survey == "RDA" ~ dplyr::case_when(
        score  < 48 ~ "Distress",
        score >= 48 ~ "Non-Distress"
      ),
      survey == "MOCA"  ~ dplyr::case_when(
        score >= 26 ~ "Normal",
        score < 26 ~ "Impaired"
      ),
      survey == "BRS" ~ dplyr::case_when(
